...
- Renovation Needs Note: ${
    property.renovationNeeds || "Ingen spesifikke notater"
  }
      ...
      items: {
        ...
        properties: {
          title: {
            type: Type.STRING,
            description:
              "A short, catchy title for the renovation suggestion in Norwegian (e.g., 'Moderniser Kjøkkenet').",
          },
          description: {
            type: Type.STRING,
            description:
              "A one or two sentence description in Norwegian of the suggested renovation and its benefits.",
          },
          cost_level: {
            type: Type.STRING,
            description:
              "The estimated cost level for the renovation. Must be one of: 'Lav', 'Middels', 'Høy'.",
          },
        },
      },
    },
...
export const getRenovationSuggestions = async (
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "You are a helpful assistant for real estate evaluation. You provide concise, practical, and actionable renovation suggestions in Norwegian. Your goal is to help users identify improvements that could increase a property's value and appeal. Respond only with the requested JSON.",
